...
/**
 * Taken from https://github.com/joseprio/xBRjs
 * slightly addapted to work in nodejs and use my own ImageData polyfil
 * removed the image/canvas input and converted to ImageData only
 */
...
/*
//TODO unused and has error, delete?
...
export function applyXBRraw(originalImageData: ImageData, factor: number,blendColors:boolean) {

	var originalPixelView = new Uint32Array(originalImageData.data.buffer);

	var sourceWidth = originalImageData.width;
	var sourceHeight = originalImageData.height;
	var scaledWidth = originalImageData.width * factor;
	var scaledHeight = originalImageData.height * factor;
	/**
	 ...
	 **/
	var scaledImageData = new ImageData(scaledWidth, scaledHeight);
