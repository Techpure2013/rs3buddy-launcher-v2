...
import { ModelAttribute } from "./avautils";
import { bufferToBase64, TextEncoderPolyfill } from "../../util/util";
	...
	async convert(options?: { singlefile?: boolean, baseurl?: string, glb?: boolean, imgmimetype?: "image/png" | "image/webp" } | undefined) {
		...
		for (let i in json.images!) {
			...
			if (mergebuffers) {
				...
				img.uri = bufferToDataUrl(imgfile, imgmimetype);
			} else {
		...
		if (!mergebuffers) {
			for (let i = 0; i < buffers.length; i++) {
				let url = "";
				if (inlinedata) {
					url = bufferToDataUrl(new Uint8Array(buffers[i].buffer, buffers[i].byteOffset, buffers[i].byteLength));
				}
				else {
...
function bufferToDataUrl(buf: Uint8Array, mime = "application/octet-stream") {
